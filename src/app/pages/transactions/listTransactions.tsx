...
import {
  useByIdRecurringTransactionsData,
  useByUserTransactionsData,
} from "@/hooks/transactions/useByUserTransactionsData";
...
export default function ListTransactions() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      // depois tenho q arrumar esse negocio porq o user pode vir como null
      if (!user) {
        throw new Error("Usuário não autenticado.");
      }
  ...
  const handleRowClick = async (
    ...
    if (expandedTransactionId === transactionId) {
      ...
      if (recurringId && !recurringData[transactionId]) {
        const data = await useByIdRecurringTransactionsData(
          String(recurringId)
        );
        if (data) {
  ...
  // fiz um pouco diferente esse, para filrtar por categoria e pela descrição
  const filteredTransactions = transactions?.filter((transaction) => {
    const descriptionMatch = transaction.description
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
  
    const categoryName = categories[transaction.categoryId]?.toLowerCase() || "";
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={8} className="text-center py-4 text-gray-500">
                      Não foi possível encontrar nenhuma transação.
                    </td>
                  </tr>
                ) : (
                  filteredTransactions?.map((transaction) => (
                    <>
                      ...
                      >
                        ...
                        <TableCell>
                          {categories[transaction.categoryId] || "Desconhecido"}
                        </TableCell>
                        <TableCell>
                                ...
                                <TableBody>
                                  <TableRow>
                                    <TableCell className="text-sm font-semibold">
                                      {recurringData[transaction.id].id}
                                    </TableCell>
                                    <TableCell className="text-sm font-semibold">
                                      R$ {recurringData[transaction.id].amount}
                                    </TableCell>
                                    <TableCell className="text-sm font-semibold">
                                      {recurringData[transaction.id].frequency}
                                    </TableCell>
                                    <TableCell className="text-sm font-semibold">
                                      {new Date(
                                        recurringData[transaction.id].startDate
                                      ).toLocaleDateString()}
                                    </TableCell>
                                    <TableCell className="text-sm font-semibold">
                                      {recurringData[transaction.id].endDate
                                        ? new Date(
                                            recurringData[
                                              transaction.id
                                            ].endDate
